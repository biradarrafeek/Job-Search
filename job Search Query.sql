...
-- Q5. Find the names and education levels of job 
-- seekers with a "B.Sc. Computer Science" degree.
...
-- Q6. List job titles and the names of the companies offering them.
SELECT j.title, c.name AS company_name
FROM jobs j
JOIN companies c ON j.company_id = c.company_id;

-- Q7. Count the number of job seekers in each location.
SELECT location, COUNT(seeker_id) AS num_seekers
FROM job_seekers
GROUP BY location;

-- Q8. Find jobs with a salary range that starts from at least $80,000.
SELECT title, salary_range
FROM jobs
WHERE CAST(SUBSTRING_INDEX(salary_range, '-', 1) AS UNSIGNED) >= 80000;


-- Q9. List the top 3 job seekers who have applied to the most jobs.
...
-- Q14. Retrieve jobs that require a degree in Data Science.
SELECT j.title
FROM jobs j
JOIN job_seekers s ON j.location = s.location
WHERE s.education = 'B.A. Data Science';

-- Q15. List each job seeker’s latest application date.
SELECT seeker_id, MAX(application_date) AS latest_application
FROM applications
GROUP BY seeker_id;

-- Q16. Find the job with the highest number of applications.
SELECT job_id, COUNT(application_id) AS num_applications
FROM applications
GROUP BY job_id
ORDER BY num_applications DESC
...
-- Q17. Calculate the percentage of job applications per location.
SELECT location, (COUNT(application_id) / (SELECT COUNT(*) FROM applications) * 100) AS application_percentage
FROM jobs j
JOIN applications a ON j.job_id = a.job_id
GROUP BY location;

-- Q18. List job seekers who have applied to multiple jobs in different locations.
SELECT seeker_id
FROM applications a
...
-- Q19. Find job seekers with the maximum number of applications using a subquery.
SELECT seeker_id
FROM applications
GROUP BY seeker_id
...
-- Q20. Find the rank of each job seeker based on the number of applications they submitted, with the highest number of applications ranked first.
SELECT seeker_id,
...
-- Q21. Retrieve each job’s average salary, as well as the difference between each job’s salary and the average salary across all jobs.
SELECT job_id,
       title,
       job_salary,
       avg_salary_all_jobs,
...
-- Q22. Calculate the cumulative number of applications for each job over time, ordered by application date.
...
-- Q23. Determine the running total of applications submitted by each job seeker, ordered by application date.
SELECT seeker_id,
       application_date,
...
-- Q24. Rank each job by the number of applications received within each company, with the most-applied-to job ranked first.
